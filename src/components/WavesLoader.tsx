...
const WavesLoader = ({ size = "md", className = "" }: WavesLoaderProps) => {
  const px = SIZES[size];
  ...
  return (
    <div className={`flex items-center justify-center text-muted-foreground ${className}`}>
      <svg width={px * 0.65} height={px * 0.55} viewBox="0 0 22 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path ref={pathRefs[0]} d={STATIC_PATHS[0]} stroke={strokeColor} strokeWidth="2" strokeLinecap="round" />
        <path ref={pathRefs[1]} d={STATIC_PATHS[1]} stroke={strokeColor} strokeWidth="2" strokeLinecap="round" />
        <path ref={pathRefs[2]} d={STATIC_PATHS[2]} stroke={strokeColor} strokeWidth="2" strokeLinecap="round" />
      </svg>
    </div>
  );
};
