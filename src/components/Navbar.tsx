  ...
  return (
    <header className="h-14 border-b border-border bg-background flex items-center px-4 shrink-0 transition-colors duration-300">
      ...
      <div className="min-w-[200px] flex items-center justify-end gap-2">
        ...
        <Button variant="ghost" size="icon" onClick={onToggleTheme} className="h-8 w-8 relative overflow-hidden">
          <Sun className={`h-4 w-4 absolute transition-all duration-300 ease-in-out ${isDark ? 'rotate-0 scale-100 opacity-100' : '-rotate-90 scale-0 opacity-0'}`} />
          <Moon className={`h-4 w-4 absolute transition-all duration-300 ease-in-out ${isDark ? 'rotate-90 scale-0 opacity-0' : 'rotate-0 scale-100 opacity-100'}`} />
        </Button>
      </div>
    </header>
  );
